...
class AirbnbSpider(scrapy.Spider):
        ...
        if homes is None:
            try:
                homes = data.get('explore_tabs')[0].get(
                    'sections')[1].get('listings')
            except:

                try:
                    homes = data.get('explore_tabs')[0].get(
                        'sections')[3].get('listings')
                except IndexError:
                    try:
                        homes = data.get('explore_tabs')[0].get(
                            'sections')[2].get('listings')
                    except:
                        raise CloseSpider(
    ...
    def parse_details(self, response):
        ...
        listing['check_in'] = self.checkin
        listing['check_out'] = self.checkout
